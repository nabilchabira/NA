...
constexpr uint32_t DEFAULT_SAMPLER_SEED = LLAMA_DEFAULT_SEED;
...
static common_sampler *new_sampler(
        float temperature, int32_t top_k, float top_p, float repeat_penalty, uint32_t seed) {
...
Java_com_arm_aichat_internal_InferenceEngineImpl_prepare(JNIEnv * /*env*/, jobject /*unused*/) {
    ...
    g_sampler = new_sampler(DEFAULT_SAMPLER_TEMP, DEFAULT_SAMPLER_TOP_K, DEFAULT_SAMPLER_TOP_P,
                            DEFAULT_SAMPLER_REPEAT_PENALTY, DEFAULT_SAMPLER_SEED);
    return 0;
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_processUserPrompt(
    ...
    // Apply per-generation sampling parameters (re-creates sampler state)
    common_sampler_free(g_sampler);
    g_sampler = new_sampler(jtemperature, jtop_k, jtop_p, jrepeat_penalty, (uint32_t) jseed);
